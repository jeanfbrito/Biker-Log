...
class CalibrationService(context: Context) {
    ...
    private suspend fun processCalibration() {
        ...
        val wasStable =
            accelStd.all { it < stabilityThreshold } &&
                gyroStd.all { it < 0.5f } // Relaxed for handheld/motorcycle mount

        android.util.Log.d(
            "CalibrationService",
            "Stability check: accelStd=${accelStd.contentToString()}, threshold=$stabilityThreshold, wasStable=$wasStable",
        )

        if (!wasStable) {
            android.util.Log.e(
                "CalibrationService",
                "Device was moving: accelStd=${accelStd.contentToString()}, gyroStd=${gyroStd.contentToString()}",
            )
