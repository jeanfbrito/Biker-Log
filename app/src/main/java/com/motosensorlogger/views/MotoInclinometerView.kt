 ...
 */
class MotoInclinometerView @JvmOverloads constructor(
    context: Context,
    ...
    private val motorcyclePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.STROKE
        strokeWidth = 3f
     ...
     */
    fun setCalibrationOffsets(pitchOffset: Float, rollOffset: Float) {
    ...
    private fun drawPitchView(canvas: Canvas, centerX: Float, centerY: Float, size: Float) {
        // Draw background
        canvas.drawColor(backgroundPanelColor)
    ...
    private fun drawRollView(canvas: Canvas, centerX: Float, centerY: Float, size: Float) {
        // Draw background
        canvas.drawColor(backgroundPanelColor)
    ...
    private fun drawMotorcycleProfile(canvas: Canvas, size: Float) {
    ...
    private fun drawMotorcycleRear(canvas: Canvas, size: Float) {
        ...
        // Left wheel
        val leftWheel = RectF(-35f * s - wheelWidth/2, wheelY - wheelHeight/2, 
                              -35f * s + wheelWidth/2, wheelY + wheelHeight/2)
        canvas.drawOval(leftWheel, motorcyclePaint)
        
        // Right wheel
        val rightWheel = RectF(35f * s - wheelWidth/2, wheelY - wheelHeight/2,
                               35f * s + wheelWidth/2, wheelY + wheelHeight/2)
    ...
    private fun drawAngleText(canvas: Canvas, x: Float, y: Float, label: String, angle: Float) {
        // Background for text
